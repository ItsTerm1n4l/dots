...
function StartSection() {
  ...
  const ToolsButton = Widget.Button({
    className: 'wallpaper_button',
    cursor: 'pointer',
    child: Widget.Label('T'),
    onPrimaryClick: () => Utils.exec(`bash -c "${App.configDir}/shared/scripts/sidebar.sh toggle-tools"`)
  })
...
function EndSection() {
  ...
  const ScreenShotButton = Widget.Button({
    ...
    onPrimaryClick: () => Utils.exec(`bash -c "grim -g "$(slurp)""`)
  })
