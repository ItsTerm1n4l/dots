...
STATES_PATH=$HOME/.config/ags/yume/.states.json
...
case $1 in
  ...
  ;;
 toggle-tools)
    if [[ `cat $STATES_PATH | jq -r '.sidebar_shown'` == "tools" ]]; then
      echo `cat $STATES_PATH | jq '.sidebar_shown = "home"'` > $STATES_PATH

      exit 0
    fi

    echo `cat $STATES_PATH | jq '.sidebar_shown = "tools" | .reveal_sidebar = true'` > $STATES_PATH
  ;;
  *)
    echo "Unknown action."
  ;;
